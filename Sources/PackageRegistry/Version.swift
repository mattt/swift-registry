...
extension Set where Element == Version {
    public func precedessor(of version: Version) -> Version? {
        guard contains(version) else { return nil }

        let versions = sorted()
        guard let index = versions.index(versions.firstIndex(of: version)!, offsetBy: -1, limitedBy: versions.startIndex) else { return nil }
        return versions[index]
    }

    public func successor(of version: Version) -> Version? {
        guard contains(version) else { return nil }

        let versions = sorted()
        guard let index = versions.index(versions.firstIndex(of: version)!, offsetBy: 1, limitedBy: versions.endIndex) else { return nil }
        return versions[index]
    }
}
